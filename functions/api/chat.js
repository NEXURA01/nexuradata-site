...
5. Pour les cas suivants, transfère à un humain : forensique judiciaire, ransomware actif, urgence vitale, dossier policier, succession, mandat d'avocat. Donne le numéro 438 813-0592 et le courriel dossiers@nexuradata.ca.
...
5. Escalate to a human for: judicial forensics, active ransomware, life-critical emergency, police case, estate, attorney mandate. Give 438 813-0592 and dossiers@nexuradata.ca.
...
export const onRequest = async ({ request, env }) => {
    ...
    if (escalate) {
        messages.push({
            role: "system",
            content: locale === "en"
                ? "The user mentioned a sensitive topic (legal / forensic / emergency). After answering briefly, you MUST recommend they call 438 813-0592 or email dossiers@nexuradata.ca right away."
                : "L'utilisateur a mentionné un sujet sensible (juridique / forensique / urgence). Après une brève réponse, tu DOIS recommander d'appeler le 438 813-0592 ou d'écrire à dossiers@nexuradata.ca sans tarder."
        });
    }
    ...
    if (!env?.AI || typeof env.AI.run !== "function") {
        return json({
            ok: false,
            message: locale === "en"
                ? "Assistant offline. Please call 438 813-0592 or email dossiers@nexuradata.ca."
                : "Assistant indisponible. Appelez le 438 813-0592 ou écrivez à dossiers@nexuradata.ca."
        }, { status: 503 });
    }
    ...
    try {
        ...
        const reply = sanitize(out?.response || out?.result?.response || "", 4000).trim()
            || (locale === "en"
                ? "Sorry, I couldn't form a clear answer. Please call 438 813-0592."
                : "Désolé, je n'ai pas pu formuler de réponse claire. Appelez le 438 813-0592.");
        ...
        return json({
            ok: false,
            message: locale === "en"
                ? "Assistant error. Please call 438 813-0592."
                : "Erreur assistant. Appelez le 438 813-0592."
        }, { status: 502 });
    }
};
